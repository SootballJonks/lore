...
/*  THIS FILE HOUSES QUERY FUNCTIONS IN THE STYLE OF LIGHTBNB.
    I HAVE NOT BEEN ABLE TO GET THESE TO WORK WITH OUR ROUTER FORMATTING.
    THESE WILL STAY HERE AS A STRETCH GOAL,
    OTHERWISE THIS FILE IS COMPLETELY USELESS.
    -Ana
*/
...
const getAllStories = () => {
  ...
  return db
    ...
    .then((data) => data.rows) //we only need to return the object array for all stories
    .catch((err) => {
      console.error("query error:", err.stack);
    });
};
...
const newStory = (entry) => {
  const queryString = `
    ...
    RETURNING *;
  `;
  const values = [entry.user_id, entry.title, entry.text, entry.tags];
